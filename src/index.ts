    ...
    function carregarBanco(): BancoMemoria {
        ...
        try {
            ...
            console.log('Arquivo de dados inválido. O sistema iniciou com um banco limpo para evitar falha.');
            return bancoPadrao();
        }
    }
    ...
    async function perguntarObrigatorio(texto: string, limite = 80): Promise<string> {
        while (true) {
            ...
            if (valor.length > limite) {
                console.log(`Use no máximo ${limite} caracteres.`);
                continue;
            }
            return valor;
        }
    ...
    async function perguntarNumero(texto: string, minimo = 1): Promise<number> {
        while (true) {
            ...
            console.log(`Informe um número inteiro maior ou igual à ${minimo}.`);
        }
    }
    ...
    async function perguntarCodigo(texto: string): Promise<string> {
        while (true) {
            ...
            console.log('Use apenas letras, numeros, hÍfen(-) ou underline(_).');
        }
    }
    ...
    async function perguntarUsuario(texto: string): Promise<string> {
        while (true) {
            ...
            console.log('Use apenas letras, numeros, ponto, hífen(-) ou underline(_).');
        }
    }
    ...
    function exigirPermissao(usuario: Funcionario, ...permissoes: NivelPermissao[]): boolean {
        ...
        console.log('Acesso negado para seu nível de permissão.');
        return false;
    }
    ...
    async function cadastrarFuncionario(banco: BancoMemoria): Promise<void> {
        ...
        if (banco.funcionarios.some((funcionario) => funcionario.id === id)) {
            console.log('Já existe funcionario com esse ID.');
            return;
        }
        const usuario = await perguntarUsuario('Usuário de login: ');
        if (banco.funcionarios.some((funcionario) => funcionario.usuario.toLowerCase() === usuario.toLowerCase())) {
            console.log('Já existe funcionario com esse usuario.');
            return;
        }
    ...
    async function adicionarPeca(banco: BancoMemoria): Promise<void> {
        ...
        console.log(aeronave.adicionarPeca(new Peca(nome, tipo, fornecedor, status)) ? 'Peça adicionada.' : 'Já existe uma peça com esse nome nesta aeronave.');
    }
    ...
    async function atualizarPeca(banco: BancoMemoria): Promise<void> {
        ...
        console.log(aeronave.atualizarStatusPeca(nome, status) ? 'Status atualizado.' : 'Peça não encontrada.');
    }
    ...
    async function adicionarEtapa(banco: BancoMemoria): Promise<void> {
        ...
        console.log(aeronave.adicionarEtapa(new Etapa(nome, prazo)) ? 'Etapa adicionada ao fim da ordem de produção.' : 'Já existe etapa com esse nome nesta aeronave.');
    }
    ...
    async function associarFuncionarioEtapa(banco: BancoMemoria): Promise<void> {
        ...
        console.log(etapa.adicionarFuncionario(funcionario) ? 'Funcionário associado.' : 'Funcionário já estava associado.');
    }
    ...
    async function gerarRelatorio(banco: BancoMemoria): Promise<void> {
        ...
        if (!aeronave.prontaParaEntrega()) {
            const confirmar = (await perguntarComPadrao('A aeronave ainda possui pendências. Gerar mesmo assim? (S/N): ', 'n')).toLowerCase();
    ...
    async function menu(): Promise<void> {
        ...
        while (!sair) {
            ...
            console.log('6 - Adicionar peça');
            console.log('7 - Atualizar status de peça');
